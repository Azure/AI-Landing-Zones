...
| Core AI  | Azure OpenAI deployments | LLM & embedding models | Yes | Extra models / regions | PTU + pay-as-you-go pairing |
...
| Data | Azure Cosmos DB | Chat state, agent memory, RAG metadata | Yes | Analytical store | Multi-region optional |
...
Best practice: Reserve at least /27 per high-traffic private endpoint set; document growth plan for vector database / search scaling.
...
Best practice: Keep hub SKU `S0` initially; plan capacity when enabling multi-project evaluation flows. Enforce cost allocation tags at project level. Use project connections for least-privilege secrets retrieval rather than broad role assignments on the hub.
...
Best practice: For token control, enforce custom policies (rate, quota) in named policy fragments; publish model endpoints as products that require subscription approval.
...
Best practice: Start WAF in Detection in lower environments; production in Prevention with OWASP 3.2 baseline. Increase file upload limit only when required for large embedding ingestion.
...
Best practice: Use UDR to force 0.0.0.0/0 through firewall only when mandated; otherwise rely on service tags + private endpoints to reduce cost and latency.
...
Best practice: Standardize scale rules (HTTP concurrency, KEDA event sources) and secrets retrieval via Key Vault references across all Container Apps definitions (`containerAppsList` in Bicep vs downstream module definitions in Terraform).
...
Best practice: Pre-create containers for chat sessions and embedding metadata with partition keys (`/sessionId`, `/docCollection`) to avoid hot partitions.
...
Best practice: Decide early on single vs dual index (hybrid vs vector-only) and document per-skill index schema.
...
Best practice: Define lifecycle rules to archive chat transcripts / logs over 90 days to cooler tiers or delete per retention policy.
