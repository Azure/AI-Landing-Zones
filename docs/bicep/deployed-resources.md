...
!!! tip "How to opt out of the duplicate Foundry data plane"
    - **Bring your own** — set `aiSearchResourceId`, `aiFoundryStorageAccountResourceId`, and `aiFoundryCosmosDBAccountResourceId` to existing resource IDs of a **dedicated Foundry-only data plane** that belongs exclusively to this Foundry account / landing zone. **Never co-mingle the Agent Service data plane with anything else** — not with the workload's own Search / Storage / Cosmos, not with another workload's, and not with another Foundry instance's. Co-mingling collapses the blast radius (a runaway agent index or thread blob can starve unrelated queries), couples IAM (every co-tenant identity ends up needing — and getting — data-plane access to agent state), and breaks lifecycle ownership (no team can drop / recreate its own data services without coordinating across the co-tenants). The safest pattern is to keep the defaults and let each landing zone provision its own Foundry-dedicated trio; only use BYO when the **same Foundry instance** already has a trio provisioned out-of-band that you want to reattach to.
